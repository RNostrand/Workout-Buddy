    ...
    <div v-if="dataLoaded">
      ...
      <div class="flex flex-col items-center p-8 rounded-md shadow-md bg-light-grey relative">
        ...
        <div v-if="loggedIn" class="flex absolute left-2 top-2 gap-x-2">
          ...
          <!--- Edit Image --->
          <div @click="editMode" class="h-7 w-7 rounded-full flex justify-center items-center cursor-pointer bg-navy shadow-lg">
            <img class="h-3.5 w-auto" src="@/assets/images/pencil-light.png" alt=""/>
          </div>

          <!--- Delete Image --->
          <div @click="deleteWorkout" class="h-7 w-7 rounded-full flex justify-center items-center cursor-pointer bg-navy shadow-lg">
            <img class="h-3.5 w-auto" src="@/assets/images/trash-light.png" alt=""/>
          </div>
        </div>

        <!--- Cardio Image --->
        <img v-if="data.workoutType === 'cardio'" class="h-24 w-auto" src="@/assets/images/running-navy.png" alt=""/>

        <!--- Strength Image --->
        <img v-else class="h-24 w-auto" src="@/assets/images/dumbbell-navy.png" alt=""/>

        <!--- Workout Type --->
        <span class="mt-6 py-1.5 px-5 text-xs text-white bg-navy rounded-lg shadow-md">{{data.workoutType}}</span>
        ...
        <div class="w-full mt-6">
          ...
          />
          <h1 v-else class="text-navy text-2xl text-center">{{data.workoutName}}</h1>
        </div>

      </div>

      <!--- Exercises --->
      <div class="flex flex-col mt-10 p-8 rounded-md item-center bg-light-grey shadow-lg mb-40">
        ...
        <div v-if="data.workoutType === 'strength'" class="flex flex-col w-full gap-y-4">
          ...
          :key="index">
            ...
            <!--- Delete Image --->
            <img @click="deleteExercise(item.id)" v-if="edit" class="h-4 w-auto absolute -left-5 cursor-point" src="@/assets/images/trash-navy.png" alt=""/>

          </div>
          
          <button @click="addExercise" v-if="edit" type="button" class="py-2 px-6 rounded-md self-start text-sm text-white 
      bg-navy duration-200 border-solid border-2 border-transparent
      hover:border-navy hover:bg-navy hover:text-navy">Add Exercise</button>
        </div>
        ...
        <div v-else class="flex flex-col w-full gap-y-4">
          ...
          :key="index">
            ...
            <div class="flex flex-col flex-2">
              <label for="cardioType" class="mb-1 text-sm text-navy">Type</label>
              <select
              v-if="edit"
              type="text"
              class="p-2 w-full text-gray-500 focus:outline-none"
      ...
      <!--- Update --->
      <button @click="update" v-if="edit" type="button" class="mt-10 py-2 px-6 rounded-md self-start text-sm text-white 
      bg-navy duration-200 border-solid border-2 border-transparent
      hover:border-navy hover:bg-navy hover:text-navy">Update Workout</button>
...
export default {
  ...
  setup() {
    ...
    const getData = async() => {
      try{
        if(user){
          const {data: workouts, error} = await supabase.from('Workouts').select('*').eq('user', user.id).eq('id', currentId);
          if (error) throw error;
          data.value = workouts[0];
          dataLoaded.value = true;
        }
        else{
          const {data: workouts, error} = await supabase.from('Workouts').select('*').eq('user', demoId).eq('id', currentId);
          if (error) throw error;
          data.value = workouts[0];
          dataLoaded.value = true;
        }
    ...
    return {data, dataLoaded, errMsg, statusMsg, loggedIn, edit, deleteWorkout, editMode, addExercise, deleteExercise, update};
  },
};
</script>
